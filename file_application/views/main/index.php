...
<?
    ...
    if ($counts) { 
        foreach ($counts as $value) {
            $new_date = date("d/m/Y", strtotime($value['departure_date']));
            $title = $value['number'] . ' ' . lang('m.event');
...
<script>
  $(document).ready(function() {
    var dayToday = '<?= date("d/m/Y"); ?>';
    var events = [  <?=  $count_text ?> 
                  { date: dayToday, 
                    color: 'rgba(215, 204, 0, 1)',
                    title: today,
                  }]; 
    var dayNamesCalendar  = [  "Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi", "Pazar"], 
        monthNamesCalendar = [ "Ocak", "Şubat", "Mart", "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül", "Ekim", "Kasım", "Aralık" ];
    if( strcmp(lang, "en") == 0 ){
      dayNamesCalendar  = [ "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday" ]; 
      monthNamesCalendar = [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ];
    ...
    $('#calendari_lateral1').bic_calendar({
      ...
      multiSelect: false,
      //set day names
      dayNames: dayNamesCalendar,
      //set month names
      monthNames: monthNamesCalendar,
      //show dayNames
      showDays: true,
