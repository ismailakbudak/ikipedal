  ...
  <div class='well trip-content'>
	...
	$val .= "<div class='row row-side verified-title'>
        ...
        <div class='row row-side' style='margin-top:30px'>
          <a href='" . new_url('user/show/' . urlencode(base64_encode($user['id']))) . "' class='text-primary' > → " . lang('sd.show-profil') . "</a>
        </div>
      </div>
    </fieldset>
  </div>
...
function photoCheckUser($user) {
	...
	if (strcmp($foto, "") != 0) {
		$path = realpath(getcwd() . "/public/assets/");// get path
...
function offerRideError() {
	...
	"er.blank_name='" . lang('g.blank_name') . "';" .
	"er.locations='" . lang('g.locations') . "';" .
