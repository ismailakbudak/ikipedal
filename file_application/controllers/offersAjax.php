...
class OffersAjax extends CI_Controller {
	...
	public function createOffer2() {
		...
		{ $this->form->check(lang('oc.return_days'), 'return_days', 'required|is_day|xss_clean');
		}
		// check post data
	...
	private function saveOffer($user_id) {
		...
		if ($this->form->get_result()) {
			...
			if (trim($post_way_points) != "") {// way points is null or not
				...
				$distances = explode('?', $post_input_distances);// way points distance split
				$times = explode('?', $post_input_times);// way points distances time split
				$waypoint = array('departure_place' => $event['origin'],
				...
				$waypoint = array('departure_place' => $points[count($points) - 1],
					'arrivial_place' => $event['destination'],
			...
			// ana yolculuğu ekle
			$ways_offer[] = array('departure_place' => $event['origin'],
				'arrivial_place' => $event['destination'],
				'distance' => $event['total_distance'],
				'time' => $event['total_time'],
				'price_class' => $event['price_class'],
				'price' => $event['price_per_passenger'],
			);

			foreach ($ways_offer as &$value) {
				$result = getLocation($value, $locations);
			}
			if (strcmp($event['trip_type'], "1") == 0) {// if trip type is rutin trip there is departure days and return days
				...
				if (count($go) > 0) {
					...
					if (strcmp("1", $event['round_trip']) == 0) {// if trip is two way there is some return days
