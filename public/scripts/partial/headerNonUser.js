       ...
       $.ajax({
         ...
         success: function(result) {
           ...
           if (strcmp(answer.status, 'login') == 0) {
             ...
             HataMesajModal(thisID, er.ban);
           } else if (strcmp(answer.status, 'error') == 0) {
             HataMesajModal(thisID, er.wrong_data);
           } else if (strcmp(answer.status, 'error2') == 0) {
             HataMesajModal(thisID, answer.message);
           } else {
       ...
       $.ajax({
         ...
         success: function(result) {
           ...
           if (strcmp(answer.status, 'success') == 0) {
             location.reload(true);
           } //window.location = base_url + 'login/?result=1';
           else if (strcmp(answer.status, 'emailusing') == 0) {
             HataMesajModal(thisID, er.email_using);
           } else if (strcmp(answer.status, 'fail') == 0) {
             HataMesajModal(thisID, er.sign_failed);
           } else if (strcmp(answer.status, 'mistake') == 0) {
             HataMesajModal(thisID, er.wrong_cap);
             createCaptcha();
             cap.val("");
             cap.focus();
           } else if (strcmp(answer.status, 'error') == 0) {
