         $(function() { 
             ...
             $.ajax({
               ...
               success: function(answer) {
                 ...
                 if (strcmp(result.status, 'success') == 0) {
                   ...
                   message.val("");
                   setTimeout(function() {
                     $('#contact-driver-modal').modal('toggle');
                   }, 500);
                 } else if (strcmp(result.status, 'mistake') == 0) {
                   HataMesajModal($('#send-message'), result.text)
                 } // show bottom alert
                 else if (strcmp(result.status, 'fail') == 0) {
                   HataMesajModal($('#send-message'), result.text)
                 } // show bottom alert
                 else if (strcmp(result.status, 'error') == 0) {
                   HataMesajModal($('#send-message'), result.message)
                 } // show top
                 else {
                   HataMesajModal($('#send-message'), er.error_send)
                 } // show bottom alert  
               },
               error: function() {
           ...
           $("#delete-modal .btn-primary").on('click', function() {
             ...
             if (strcmp(offer_id, "-1") != 0) {
               ...
               if (strcmp(result.status, 'success') == 0) {
                 window.location = base_url + "offer"
               } // show bottom alert 
               else if (strcmp(result.status, 'fail') == 0) {
                 HataMesajModal(id, result.text)
               } // show bottom alert
               else if (strcmp(result.status, 'error') == 0) {
                 HataMesajModal(id, result.message)
               } // show top
               else {
                 HataMesajModal(id, er.error_send)
               } // show bottom alert  
             } else {
               HataMesajModal($(this), er.error_occurred)
             }
             return false;
           });
